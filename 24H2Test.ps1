...
#################################################################
#   [PreOS] HP-detectie en OSDCloud opties bepalen
#################################################################

# Standaard waarden (voor non-HP of geen HPIA)
$HPBIOSUpdate           = $false
$HPTPMUpdate            = $false
$HPCMSLDriverPackLatest = $false
...
try {
    ...
    if (Test-HPIASupport) {
        Write-Host -ForegroundColor Cyan "HP device gedetecteerd ($Model / $Product). HPIA/BIOS/TPM driverlogica wordt geactiveerd."

        $HPBIOSUpdate           = $true
        $HPTPMUpdate            = $true
        $HPCMSLDriverPackLatest = $true
        $WindowsUpdateDrivers   = $false
    }
    else {
        Write-Host -ForegroundColor DarkGray "Geen HPIA-ondersteuning. Standaard Windows Update drivers worden gebruikt."
    }
}
catch {
    Write-Host -ForegroundColor Red "Fout bij HP/HPIA-detectie: $($_.Exception.Message). Gebruik standaard driverlogica."
    $HPBIOSUpdate           = $false
    $HPTPMUpdate            = $false
    $HPCMSLDriverPackLatest = $false
    $WindowsUpdateDrivers   = $true
...
$Global:MyOSDCloud = [ordered]@{
    ...
    WindowsUpdateDrivers  = [bool]$WindowsUpdateDrivers
    ...
    HPIAALL               = [bool]$false
    HPCMSLDriverPackLatest = [bool]$HPCMSLDriverPackLatest
}
...
start /wait powershell.exe -NoProfile -Command "iex (irm 'https://functions.osdcloud.com'); osdcloud-updatewindows" >> "%logfile%" 2>&1

...
#=======================================================================
# Init Vars
$HPTPM = $false; $HPBIOS = $false; $HPIADrivers = $false
...
# OSDCloud functions (best-effort) - Nodig voor de HP Check, anders geen Test-HPIASupport
...
if (-not $IsVM -and $InternetConnection -and $FunctionsLoaded -and $IsHP -and $HasTestHPIA) {
    ...
    $HPTPM = $true
    $HPBIOS = $true
    Write-Host "HP systeem gedetecteerd. HPIA, TPM en BIOS updates ingeschakeld." -ForegroundColor Yellow
}
else {
    # Log redenen waarom HP/HPIA overgeslagen wordt
    ...
    elseif (-not $IsHP) { Write-Host "Geen HP hardware; HP/HPIA overgeslagen." -ForegroundColor DarkYellow }
}
